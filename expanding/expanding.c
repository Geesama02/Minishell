...
/*   Updated: 2024/07/08 17:34:11 by maglagal         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	int i)
{
	...
	if (!new_env)
		return (free_cmds(tokens), free(tokens),
			free_envs(head), free_cmds(cmds), free(cmds), exit(1));
	if (prev)
		prev->next = new_env;
	else if (!prev && i == 1)
		*head = new_env;
	...
	if (!new_env->env_name)
		return (free_cmds(cmds), free(cmds), free_envs(head),
			free_cmds(tokens), free(tokens), exit(1));
	if (ft_strchr(cmds[1], '$'))
...
void	add_or_append(char **cmds, t_env_vars **head,
	char **tokens, int i)
{
	char		*env_name;
	t_env_vars	*tmp;
	...
	{	
		...
		if (!env_name)
			return (free_cmds(cmds), ft_close(tokens, head), exit(1));
...
void	add_env_var(char **tokens, int nbr_envs, t_env_vars **head)
{
	...
	while (i <= nbr_envs)
	{
		...
		add_or_append(cmds, head, tokens, i);
		free_cmds(cmds);
		free(cmds);
		i++;
	}
